...
conditions = [["<{sample} --> [A1]>. :|:", "<{left} --> [B1]>. :|:", "<{right} --> [B2]>. :|:", "^left"],
              ["<{sample} --> [A1]>. :|:", "<{left} --> [B2]>. :|:",
                  "<{right} --> [B1]>. :|:", "^right"],
              ["<{sample} --> [A2]>. :|:", "<{left} --> [B1]>. :|:",
                  "<{right} --> [B2]>. :|:", "^right"],
              ["<{sample} --> [A2]>. :|:", "<{left} --> [B2]>. :|:", "<{right} --> [B1]>. :|:", "^left"]]
...
seq = [0, 1, 2, 3]
print("Learning A1->B1 and A2->B2")
print("***************")
print()
...
for _ in range(8):
    ...
    for i in s:
        ...
        if "operator" not in response["executions"][0]:
            print("INCORRECT: " + op)
            NAR.AddInput("G. :|: {0.0 0.9}", Print=True)
        else:
            op = response["executions"][0]["operator"]
